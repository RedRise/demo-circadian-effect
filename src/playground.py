...
# plot volume by hour
hmean = hdf.groupby('Hour').apply(lambda x: x['Volume ETH'].mean()).reset_index(name='Mean Volume')
...
def compute_class(hdf, closing_hour, day_length, should_sort=True):
  ...
  pdf['Class'] = pdf.apply(lambda x: 'day' if x['Hour'] == closing_hour else 'night', axis=1)
...
fig = tools.make_subplots(rows=1, cols=2 ,print_grid=False,  shared_yaxes=True, subplot_titles=("Day", "Night"))

ldf = rdf.loc[rdf["Class"]=="day"]
fig.append_trace(go.Heatmap(x=ldf["DayLength"], y=ldf["Close"], z= ldf[varName], coloraxis='coloraxis'), 1, 1)           

ldf = rdf.loc[rdf["Class"]=="night"]
fig.append_trace(go.Heatmap(x=ldf["DayLength"], y=ldf["Close"], z= ldf[varName], coloraxis='coloraxis'), 1, 2)           

fig = fig.update_layout(title_text= PAIR + " " + varName + " during Day and Night", coloraxis = {'colorscale':'thermal'})
fig.show()
...
tdf = pdf.reset_index()[['Date', 'Class', 'Return']].set_index("Date").fillna(0).groupby('Class').apply(lambda x: x['Return'].cumsum()).reset_index(name='Return')
px.line(tdf, x="Date", y="Return", color="Class", title="{0} cumulated returns Day vs. Night. Close: {1}h, Day Length: {2}h".format(PAIR, ch, dl)).show()
